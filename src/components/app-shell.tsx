    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:pl-64">
        ...
        <header className="no-print sticky top-0 z-20 border-b border-border bg-background/90 backdrop-blur">
          <div className="flex items-center gap-2.5 px-3 py-2.5 sm:px-4 sm:py-3 lg:px-8">
            <span className="flex items-center gap-2 lg:hidden shrink-0">
              <span className="flex size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground">
                <Scissors className="size-4" />
              </span>
              <span className="max-w-[7.5rem] xs:max-w-[9.5rem] truncate font-display text-xs font-bold sm:text-sm">
                {business?.name ?? "Mon atelier"}
              </span>
            </span>
            <button
              onClick={() => setSearchOpen(true)}
              className="ml-auto flex flex-1 items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:border-primary/40 sm:px-4 sm:py-2 sm:text-sm lg:max-w-sm"
              aria-label="Rechercher"
            >
              <Search className="size-3.5 sm:size-4 shrink-0" />
              <span className="truncate">Rechercher client, commande…</span>
        ...
        <main className="px-3 pb-24 pt-4 sm:px-6 sm:pb-28 sm:pt-6 lg:px-8 lg:pb-12 max-w-full overflow-x-hidden">
          {children}
        </main>
      </div>
      ...
      <Link
        ...
        className="no-print fixed bottom-20 right-4 z-40 flex size-13 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-raised transition-all hover:scale-105 active:scale-95 lg:hidden"
      ...
      {/* Navigation basse — téléphone avec cibles tactiles larges et safe area */}
      <nav className="no-print fixed inset-x-0 bottom-0 z-30 border-t border-border bg-card/95 pb-[max(env(safe-area-inset-bottom),0.35rem)] pt-1 backdrop-blur lg:hidden">
        <div className="flex items-center justify-around">
          {MOBILE_NAV.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex flex-1 flex-col items-center justify-center py-1.5 text-[0.68rem] transition-all active:scale-90 ${
                  active
              ...
              >
                <span
                  className={`flex items-center justify-center rounded-full px-3 py-0.5 transition-all ${
          ...
          >
            ...
            <span className="mt-0.5">Atelier</span>
          </Link>
        </div>
      </nav>
